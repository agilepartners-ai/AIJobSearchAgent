...
export class DocxResumeGenerator {
  ...
  private static readonly BLACK_TEXT_COLOR = "000000";
  ...
  private forceBlackTextColor(target: unknown): void {
    if (!target || typeof target !== 'object') {
      return;
    }

    if (Array.isArray(target)) {
      target.forEach((item) => this.forceBlackTextColor(item));
      return;
    }

    const mutableTarget = target as Record<string, unknown>;
    Object.keys(mutableTarget).forEach((key) => {
      const value = mutableTarget[key];

      if (key === 'color' && typeof value === 'string') {
        mutableTarget[key] = DocxResumeGenerator.BLACK_TEXT_COLOR;
        return;
      }

      this.forceBlackTextColor(value);
    });
  }
  ...
  public async generateDocument(): Promise<Buffer> {
    ...
    this.forceBlackTextColor(documentConfig);
