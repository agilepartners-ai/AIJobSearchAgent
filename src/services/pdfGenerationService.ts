 ...
 *
 * NOTE: This service historically handled server-driven PDF generation. The
 * project has moved to a DOCX-first workflow where DOCX documents are produced
 * and previewed in the client, and DOCX→PDF conversion is deferred to a
 * dedicated server-side conversion pipeline or a third-party service.
 *
 * The methods in this file remain for backward compatibility with older APIs
 * and will continue to attempt contacting the external resume PDF service
 * when invoked. They now emit a deprecation warning at runtime and include
 * clearer error messages recommending DOCX-first flows and server-side
 * conversion options.
 *
 * Recommended production approach:
 * - Implement a server-side DOCX→PDF conversion service (LibreOffice headless,
 *   CloudConvert, or a dedicated conversion microservice).
 * - Prefer returning DOCX blobs/URLs to the frontend and perform PDF conversion
 *   on-demand in a controlled server environment for consistent styling.
 */
...
export class PDFGenerationService {
    ...
    // Generate optimized resume PDF (DEPRECATED - prefers DOCX-first workflow)
    static async optimizeResume(
        fileId: string,
        jobDescription: string,
        options: OptimizeResumeOptions = {}
    ): Promise<Blob> {
        console.warn('[PDFGenerationService] optimizeResume is deprecated. Prefer DOCX-first flow and server-side DOCX->PDF conversion.');
        ...
        try {
            ...
            if (!this.API_KEY) {
                throw createApiError(
                    ...
                    null,
                    'Gemini API key is not configured or invalid. Please check your environment variables (NEXT_PUBLIC_GEMINI_API_KEY).'
                );
            }
            ...
            if (!contentType?.includes('application/pdf')) {
                // Accept DOCX responses in some transitional cases and warn
                if (contentType?.includes('openxmlformats-officedocument')) {
                    console.warn('[PDFGenerationService] optimizeResume received a DOCX response from the API; returning the blob as-is. For PDF output, implement server-side conversion.');
                ...
                const errorMessage = errorData?.message || errorData?.error || 'Expected PDF but received different content type. Consider using DOCX-first flow and server-side conversion.';
                throw createApiError(
                    endpoint,
                    'POST',
                    requestData,
    ...
    // Generate cover letter PDF - (DEPRECATED - prefers DOCX-first workflow)
    static async generateCoverLetter(
        ...
        console.warn('[PDFGenerationService] generateCoverLetter is deprecated. Prefer DOCX-first flow and server-side DOCX->PDF conversion.');
        ...
        try {
            ...
            // Check if response is a PDF (or potentially DOCX in transitional cases)
            const contentType = response.headers.get('content-type');
            if (!contentType?.includes('application/pdf')) {
                if (contentType?.includes('openxmlformats-officedocument')) {
                    console.warn('[PDFGenerationService] generateCoverLetter received a DOCX response from the API; returning the blob as-is. For PDF output, implement server-side conversion.');
                    const blob = await response.blob();
                    return blob;
                }
    ...
    static downloadBlob(blob: Blob, filename: string): void {
        try {
            const safeName = PDFGenerationService.sanitizeFilename(filename);
    ...
    // Sanitize filename helper (local to PDFGenerationService to avoid import cycles)
    private static sanitizeFilename(name: string | undefined | null): string {
        if (!name) return 'document.pdf';
        const base = name.split(/[\\/]/).pop() || name;
        return base.replace(/[:<>\"\/\\|?*\x00-\x1F]/g, '').trim() || 'document.pdf';
    }
}
